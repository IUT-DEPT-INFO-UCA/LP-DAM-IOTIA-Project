...
Nous disposons d’une base de connaissance externe dans laquelle un grand nombre de types de dispositifs électroniques ont été enregistrés (DeviceKB). Elle comprend des modèles de montres connectées, des tensiomètres, des appareils de détection de la glycémie connectés. Elle est utilisée pour créer les questionnaires faisant référence à de tels dispositifs.
...
Un patient s’enregistre dans l’application centrale en précisant son nom et sa pathologie. Il peut pour cela être aidé par un membre de l’équipe médicale si besoin. Il peut modifier son profil, et éventuellement enregistrer un nouveau dispositif associé à son profil ; il doit alors faire référence à un type de dispositif préenregistré dans DeviceKB. Il peut également préciser le personnel médical autorisé à accéder à son profil.

Il peut également préciser le personnel médical autorisé à accéder à son profil.

#### II.1.2 Point de vue Soignant
Un soignant peut créer un questionnaire médical pour un patient donné. 
En fonction de la pathologie du patient et des dispositifs associés à son profil, un questionnaire adapté est automatiquement généré. 
Le soignant peut alors adapter la durée du questionnaire : à partir de quand le questionnaire doit être actif et pour quelle durée. Il élimine, ajoute ou modifie des questions. A partir du moment où il a validé un questionnaire celui-ci est automatiquement actif sur le téléphone du patient.

#### II.1.3 Point de vue Application sur le téléphone
L’application demande toutes les heures au questionnaire de faire ses relevés. 
Celui-ci fait les relevés automatiquement en se connectant aux dispositifs, 
et lance éventuellement des rappels au patient pour répondre à des questions textuelles ou lui demander de faire des relevés comme par exemple prendre son taux de glycémie. 

Le questionnaire enregistre les réponses obtenues dans un « _Recueil de données_ ». 
  * Si le recueil est dans l’_état connecté_, les réponses sont envoyées aussitôt à l’application centrale. 
  * Si le recueil est dans l’_état déconnecté_, les réponses collectées sont enregistrées sur le téléphone. 
  * Lorsque la quantité des réponses collectées est supérieure à un seuil donné, le recueil passe dans un état _dégradé_. Dans ce cas, différentes stratégies de gestion des données sont possibles. Dès que le recueil retourne dans l’état connecté, toutes les réponses mémorisées sont envoyées à l’application centrale.
...
6.	Yassine garde les questions suivantes et en les spécialisant pour le patient, en particulier sur la fréquence :  
    ...
    2.	Question : Qu’avez-vous mangé aujourd’hui ? répondre matin, midi, soir
    3.	Relevé : sur la montre SGW0012345; nombre de pas ; toutes les 2 heures
    4.	Relevé : sur GlucofixTechLecteur002 ; glycémie ; matin, midi, soir
